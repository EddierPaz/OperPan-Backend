...
def _contexto_base():
    ...
    # --- Calendario de los próximos 15 días (para los modales de crear/editar) ---
    proximos_dias = []
    ...
    # --- Horarios activos con sus descansos precargados (evita N+1) ---
    horarios = (
        Horario.objects
        .filter(estado=True)
    ...
    # --- Próximo descanso de cada horario (solo fechas futuras, orden ascendente) ---
    for horario in horarios:
        futuros = sorted(
            [d for d in horario.descansos.all() if d.es_descanso and d.fecha >= hoy],
            key=lambda d: d.fecha
        )
        horario.proximo_descanso = futuros[0] if futuros else None

    # --- Resumen de asistencia de hoy, agrupado por turno ---
    turnos_hoy = {"MANANA": [], "TARDE": [], "FIJO": []}
    ...
    for horario in horarios:
        ...
        # Reutiliza los descansos ya prefetcheados en vez de hacer una query nueva
        descanso_hoy = any(
            d.es_descanso and d.fecha == hoy
            for d in horario.descansos.all()
        )
...
def horario_json(request, id):
    ...
    return JsonResponse({
        ...
        "descanso":       proximo.fecha.strftime("%d/%m/%Y") if proximo else None,
        "descanso_fecha": proximo.fecha.strftime("%Y-%m-%d") if proximo else None,
        "historial_descansos": [d.fecha.strftime("%d/%m/%Y") for d in futuros],
    })
...
def asistencia_empleado(request):
    ...
    if horario:
        ...
        asistencias = Asistencia.objects.filter(horario=horario).order_by("-fecha")
